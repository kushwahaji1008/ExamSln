    ...
    <div className="space-y-8">
      ...
      {loading ? (
        ...
        <div className="grid gap-6 xl:grid-cols-2">
          {users.map((user) => (
            <div key={user.id} className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-lg transition hover:-translate-y-1 hover:shadow-indigo-100/40">
              ...
              <div className="mt-6">
                <Link className="inline-flex items-center rounded-full bg-indigo-600 px-4 py-2 text-sm font-semibold text-white transition hover:bg-indigo-500" to={`/admin/users/${user.id}`}>
                  Edit user
                </Link>
              </div>
            </div>
