...
export default function Results() {
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-8 p-4 sm:p-6 lg:p-8 font-sans text-slate-100">
      <div>
        <h1 className="text-3xl font-extrabold tracking-tight text-white">My Results</h1>
        <p className="mt-2 text-sm text-slate-400">Review your past performance and analytical breakdowns.</p>
      </div>

      <div className="grid gap-6">
        {results.map((result) => (
          <div key={result.id} className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 rounded-2xl border border-white/10 bg-slate-900/80 p-5 backdrop-blur-xl transition hover:bg-slate-800/80">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h3 className="text-lg font-bold text-white">{result.examTitle}</h3>
                <p className="text-sm text-slate-400 mt-1">Submitted on {result.date}</p>
              </div>
            </div>

            <div className="flex w-full sm:w-auto items-center justify-between gap-8 border-t border-white/5 sm:border-0 pt-4 sm:pt-0">
              ...
              <button 
                ...
                className="flex items-center gap-2 rounded-xl bg-slate-800 px-4 py-2 text-sm font-semibold text-white transition hover:bg-sky-500"
              >
