  ...
  return (
    <div className="mx-auto max-w-7xl space-y-8 p-4 sm:p-6 lg:p-8 font-sans text-slate-100">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-extrabold tracking-tight text-white">My Courses</h1>
          <p className="mt-2 text-sm text-slate-400">Pick up right where you left off.</p>
        </div>
        <button onClick={() => navigate('/student/catalog')} className="rounded-xl bg-slate-800 px-4 py-2 text-sm font-semibold text-white transition hover:bg-slate-700">
          Browse Catalog
        </button>
      </div>
      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {courses.map((course) => (
          <div key={course.id} className="group overflow-hidden rounded-3xl border border-white/10 bg-slate-900/80 backdrop-blur-xl transition hover:border-emerald-500/30">
            {/* Header Image Area */}
            <div className="h-32 bg-slate-950 flex items-center justify-center border-b border-white/5 relative overflow-hidden">
            ...
            <div className="p-6">
              ...
              <button onClick={() => navigate(`/student/course/${course.id}`)} className="w-full flex items-center justify-center gap-2 rounded-xl bg-white px-4 py-2.5 text-sm font-bold text-slate-950 transition hover:bg-slate-200 shadow-xl shadow-white/5">
