  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-xl font-bold text-primary-foreground tracking-tight">Recommended for you</h2>
        <button 
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {courses.map((course) => (
          <div 
            key={course.id}
            className="group cursor-pointer rounded-2xl border border-border/10 bg-slate-900/50 overflow-hidden transition-all duration-300 hover:bg-slate-800 hover:border-sky-500/30 hover:shadow-xl hover:shadow-sky-500/10 hover:-translate-y-1"
            ...
            <div className="aspect-video w-full bg-slate-950 relative overflow-hidden">
              {course.thumbnailUrl ? (
                ...
                <div className="absolute inset-0 bg-gradient-to-br from-indigo-900/50 to-slate-950 flex items-center justify-center">
                  <div className="w-12 h-12 rounded-full bg-card/5 backdrop-blur-sm border border-border/10 flex items-center justify-center">
