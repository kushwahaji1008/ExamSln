    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 py-20 px-4">
      <div className="max-w-6xl mx-auto space-y-16">
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {plans.map((plan, i) => (
            <div key={i} className={`relative bg-slate-900/80 border ${plan.popular ? 'border-sky-500 shadow-lg shadow-sky-500/20' : 'border-white/10'} p-8 rounded-3xl backdrop-blur-xl flex flex-col`}>
              {plan.popular && <span className="absolute -top-4 left-1/2 -translate-x-1/2 bg-sky-500 text-white text-xs font-bold px-3 py-1 rounded-full uppercase tracking-wider">Most Popular</span>}
              ...
              <button className={`w-full py-3 rounded-xl text-sm font-semibold transition ${plan.popular ? 'bg-sky-500 text-white hover:bg-sky-400' : 'bg-slate-800 text-white hover:bg-slate-700'}`}>
                Get Started
              </button>
            </div>
          ))}
